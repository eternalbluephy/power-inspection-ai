...
class TritonPythonModel:
    def initialize(self, args):
        ...
        self.anchors, self.strides = self._make_anchors_640(self.device)

    def _make_anchors_640(self, device):
        strides = [8, 16, 32]
        feats = [640 // s for s in strides]
    ...
    def execute(self, requests):
        ...
        for req in requests:
            ...
            x2 = a[:, 0] + dist[:, 2]
            y2 = a[:, 1] + dist[:, 3]
            boxes = torch.stack([x1, y1, x2, y2], dim=1) * s
