...
class TritonPythonModel:
    def initialize(self, args):
        ...
        # anchors: (33600,2) in feature coords, strides: (33600,1)

    def _make_anchors_1280(self, device):
        # 假设 strides = [8,16,32]，输入 1280 => grids = [160,80,40]
        strides = [8, 16, 32]
        feats = [1280 // s for s in strides]  # [160,80,40]

        all_xy = []
        all_s  = []
        for s, f in zip(strides, feats):
            # grid center (x,y) in feature units, +0.5 center
            y, x = torch.meshgrid(torch.arange(f, device=device), torch.arange(f, device=device), indexing="ij")
            xy = torch.stack((x + 0.5, y + 0.5), dim=-1).view(-1, 2)  # (f*f,2)
            all_xy.append(xy)
            all_s.append(torch.full((xy.shape[0], 1), float(s), device=device))

        anchors = torch.cat(all_xy, dim=0)           # (33600,2)
        strides = torch.cat(all_s, dim=0)            # (33600,1)
    ...
    def execute(self, requests):
        resps = []
        for req in requests:
            out = pb_utils.get_input_tensor_by_name(req, "output").as_numpy()  # (1,33600,68) CPU numpy
            output = torch.from_numpy(out).to(self.device, non_blocking=False) # 搬到 GPU
            ...
            if keep.numel() > self.topk:
                ...
                topk_scores, topk_pos = torch.topk(kept_scores, k=self.topk, largest=True, sorted=False)
                keep = keep[topk_pos]
                scores = topk_scores
            ...
            x2 = a[:, 0] + dist[:, 2]
            y2 = a[:, 1] + dist[:, 3]
            boxes = torch.stack([x1, y1, x2, y2], dim=1) * s  # (N,4) pixel xyxy
